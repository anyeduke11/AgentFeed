...
| # | 能力 | 动机 | 工程量 | 前置 |
|---|---|---|---|---|
| H1 | Provider 输出契约（schema 校验） | 无校验靠事后重试，接本地/便宜模型易静默脏数据（脏 title 即证据）。**2026-09-22 归因升格**：失败 96.8% 为 parse 类、进行时、累计浪费 1.53 亿 tokens——升格 v0.2.x 首批主体（见 `docs/llm-failure-attribution.md`） | 0.5 天复现+0.5~1 天修复 | 无 |
| H2 | Prompt 一等公民（集中化） | prompt 散落（v0.1.5 后增至 8+ 处），贡献者改 prompt 要先读代码 | ~0.5 天（薄形态） | G3 ✅ 已满足 |
| H3 | 降级策略显式化 | ~~零散个案~~ **2026-09-22 归因证伪多场景假设**：失败 96.8% 单一形态（parse），唯一真实降级场景随 H1 修复消失——并入 H1 根因复现，不再单独立项 | 并入 H1 | 无 |
| H4 | Agent 友好错误 | ~~未验证~~ **E1⑤ 已实测大半**；余量=错误返回统一 hint 字段 | ~0.25 天扫尾 | E2 ✅ 已定型 |
| H5 | 冷启动种子语料 | ~~clone 空库~~ 单人本地工具冷启动为伪场景；开源时以首次运行引导替代——**转开源前置清单，不做** | — | — |

> **H 类裁决记录（2026-09-22）**：归因统计驱动——`llm_call_logs` 70,969 次调用全量分桶（`docs/llm-failure-attribution.md`）：H1 升格（27% 失败率 × 96.8% parse 类 × 日均仍在发生，全部储备中唯一六位数 token 回报项）；H3 并入 H1（多类降级假设被数据证伪）；H2/H4 维持薄做；H5 砍掉转开源清单。
...
12. I1 对话桥梁：**抽检 ✅（2026-09-22 真实链路执行）**——①零命中不编造 5/5 通过（量子色动力学/宣德青花瓷/深潜器液压/缅甸语声调/橄榄球战术五个库外问题，回答均以「库内无直接相关内容」开头，库外常识显式标注「不代表库内答案」，放宽词条引用真实 id 不虚构）②双领域角色切换 ✅（同一问题「该重点关注什么」：人工智能域答向评估治理/Agent 机制（EPFMS 等真实词条），网络安全域答向事件响应 SOP/金融安全运营——各自锚定本域 top 词条）③成本入 G1 ✅（llm_call_logs 落 8 行 success、6126 tokens）。**附带修复**：抽检发现截断信封泄漏（LLM 超 max_tokens 时 `{"answer":"` 前缀泄漏到前端）——extractAnswer 增截断兜底（剥前缀+尾垃圾），chat.test 补 2 用例，服务已重启生效
...
| 墓碑复活（内容不变 mtime 变） | ✅ **已执行（2026-09-22）**：/tmp 临时扫描根真实链路——文件入库→rm+扫描变墓碑（deleted=1）→原内容写回（mtime 新）+扫描→**复活 active=1 且总行数=1（复活非重复建行）**，对应 fix 7dd6497 |
| 软删配额护栏触发 | ✅ **已执行（2026-09-22）**：3 样本入库后 `chmod 000` 根目录（模拟卷掉线/权限变化，watcher 无 unlink 事件）→增量扫描返回 `sweepSkipped:[{reason:'empty-walk',count:3}]` 告警且 **3 文件全部保持 active（未整批墓碑化）**；权限恢复后 force 扫描正常清理（人确认通道），对应 fix d985522。附注：首轮曾用 mv 目录模拟——实测该场景走 watcher unlink 通道即时墓碑化（合理行为），护栏针对的「扫描走不到且无 watcher 事件」场景由 chmod 模拟验证 |
