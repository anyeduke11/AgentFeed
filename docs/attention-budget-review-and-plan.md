...
> 版本：v1.4（终版）· 2026-09-26
> 性质：**四方案评审收官 + 实施排期**。上游 spec ×4：元宝（v2.0 草案）、豆包（v1.0）、智谱清言（AF-RFC-2026-001）、千问（v2.0 方案 + 任务清单，docx，评审存档见 `AgentFeed改造方案-千问-评审存档.md`）——前三份已在文头打评审标记。
> 数据锚点：2026-09-26 从生产库 `app.db` 拉取的真实消化基线（见 §2）。
> 排期状态：**四份评审完成，路线与排期冻结。P0 可即刻开工。**
