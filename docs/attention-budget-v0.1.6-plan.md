# 注意力预算改造方案 v0.1.6：评审结论与修订路线（基于真实基线）

> 版本：v0.1.6（终版）· 2026-09-26
> 本文档即 AgentFeed v0.1.6 重构的方案定稿（四份外部方案——元宝/豆包/智谱清言/千问——评审过程文档已清理，全部结论沉淀于本文；评审依据与数据锚点见 §2）。
> 性质：**方案定稿 + 实施排期**。数据锚点：2026-09-26 从生产库 `app.db` 拉取的真实消化基线（见 §2）。
> 排期状态：**路线与排期冻结。P0 可即刻开工。**

---

## 0. 四方案合成结论（v1.4 终版）

四份方案共享同一诊断（注意力稀缺、门禁只有下限、压缩≠减量），能力分布互补：
...
- **智谱强在工程与成本素养**：三层摘要生成时机矩阵（hook 并蒸馏调用/brief 懒生成）是全场唯一真降本设计；marginal（已读集参照）是唯一贴合拉取式基线的评分参照系；A-1 廉价去重前置；features 灰度开关全关=对等；§11 禁止动作总表最全
- **千问强在产品与度量视野，但答错了题**：把 AgentFeed 当多用户 RSS/资讯 SaaS 设计（user_id 多用户画像、推送/打断/静默窗口、跨源事件聚合分歧点、DAU/NPS/团队 KPI 叙事、0-16 周交付周期），而真实形态是**单用户本地库、零推送、纯拉取**——模块 C（节流/推送）整层、emergency 预算、反馈模块 F 全家等约四成机制无的放矢。其六处真增量已收编（见下）
- **四案共病**：投喂/预算/消化/反馈机制全部架在「用户会来标记」的假设上，而真实基线消化信号 ≈ 零（§2）——预算面四案齐推，全部进 P3 且设 ≥50 条信号硬门槛
- **四案共识（独立趋同，可信度高）**：①先闸门后评分（千问：「先评分再闸门=精准地淹死人」与我们 P0→P2 排序互证）；②反 FOMO 的可解释性（元宝 suppressed_reason / 豆包 M6 / 智谱 omitted_count / 千问 total_available_but_hidden 四种形态）；③冷归档只改可见性不删数据

**合成公式**：入库冷却（元宝）× 蒸馏前去重（智谱 A-1）× 库侧使用衰减（豆包 M5）× 三层摘要（智谱骨架+元宝字段）× 周度聚合（豆包 M4/M6）× marginal 评分（智谱）× 出口熵减（豆包 M7）× 预算三档渐进（千问 soft/hard 双上限）× 行为红线（智谱 §11 为主+豆包 §5 并入）× 预算推迟（四案同判）

### 千问六处增量收编（v1.4）

1. `total_available_but_hidden` 内联可见：feed 响应自带「还有 N 条被拦下」——对 FOMO 心理最精准的一刀（豆包 M6 是周报、智谱 omitted_count 在 MCP，千问做成了每次拉取的内联契约）→ P3-1
2. 软/硬双上限渐进降级（80% soft 只许 hook 级 / 100% hard 停止，降级不惩罚）→ P3-1
3. digest/周报不占当日预算（激励用聚合替代实时流的机制设计）→ P3-1
4. 两指标：`dedup_compression_ratio`（原始/聚合 2.5-6× 健康区间，去重效果首次可测）→ P1-5；`budget_used_pct`（60-85% 健康，长期 100%=预算设太高）→ P3
5. 动态阈值 p70（消费越少阈值越高）→ P3+ 备选自适应机制（同样架在零信号上，不设默认）
6. 反馈权重更新规则（dismiss_forever ×0.3 / false_alarm 回退重审 / save+dwell 正样本 ≤5% 步长）→ **backlog**（等信号量 ≥50 条后评估启用）
...
| 机制 | 元宝 | 豆包 | 智谱 | 千问 | 处置 | 去向 |
|---|---|---|---|---|---|---|
| 入库侧时间抑制 | §2 L1 冷却池 | — | — | — | 采纳元宝 | P1 |
| 入库侧去重 | 冷却期 sim 判死 | — | **A-1 蒸馏前廉价去重** | 5.3 dedup/hybrid | **取智谱**（前置省成本、判冷不判死）；千问 compression_ratio 指标并入 | P1 |
| 入库侧条数配额 | — | — | A-2 daily_quota + 待审池 | — | **否决拦截**，改观测仪表 | P1（只记不拦） |
| 库侧减负 | TTL 时间衰减 | **M5 使用侧衰减** | C 分层 | — | 取豆包衰减为主 + 智谱 pinned/touch | P0 |
| 摘要分层 | L4 决策卡片 | M2 one_liner | **B 三层（hook 并蒸馏/brief 懒生成/full）** | 5.5 L1/L2/L3 | **取智谱骨架** + 元宝卡片字段 + 豆包呈现层 | P2 |
| 新颖性参照系 | 全库 novelty | 全库 novelty | **D marginal（已读集参照）** | f2 已见图谱参照（同思路） | **取智谱** | P2 |
| 消费粒度 | need_read 二值 | M2 三级模型 | hook/brief/full | L1/L2/L3 成本分档（8s/45s/180s） | 合成；千问分档成本作 P3 计额参数 | P2/P3 |
| 周维度压缩 | 无 | **M4 周度一页纸** | — | 5.6 digest | 采纳豆包；千问「digest 不占预算」豁免规则并入 | P2/P3 |
| 抑制可解释性 | suppressed_reason | **M6 安全忽略报告** | score_breakdown | **total_available_but_hidden 内联** | 四形态全收：字段（P1）+周报（P2）+推荐明细（P3）+feed 内联（P3） | P1~P3 |
| 消化信号 | T5 digestion_log | M3 digest_queue | rate_entry + one_line_use | 5.8 反馈 8 action 全家 | 简化采纳（智谱形态）；千问权重更新规则入 backlog | P2 |
| 消费间隔保护 | — | — | — | 5.7 冷却强制间隔 | **否决**（无推送面+拉取式使用，冷却期遮罩无的放矢；冷却语义已由入库侧承担） | — |
| MCP 出口 | 绑注意力预算（错） | **M7 纯熵减** | focus_mode 快照-only（错） | — | **取豆包** + include_cold | P3 |
| 预算与自适应 | §4.4 会锁死系统 | M3 900s 休眠死代码 | E daily_feed_limit 5 | 令牌桶 soft/hard 双上限 | 四案齐推；P3 启用，**取千问双上限渐进形态** + 智谱「改配置次日生效」 | P3 |
| 批次/静默/推送 | 无 | 无 | 无 | **模块 C 整层** | **整体否决**（产品错位：无推送面） | — |
| 行为红线 | 无 | **§5 禁止动作** | **§11 四域总表** | 三/6 原则 | 智谱 §11 为主 + 豆包 §5 并入；千问「可撤回的沉默」原则并入 | 全程 |
| 迁移与灰度 | /api/v2 并行 | 字段可空+一键回退 | **features 全关=对等** | — | **取智谱** + 豆包可空原则 | 全程 |
| API 信封 | — | — | — | `{code,msg,data,trace_id}` | **否决**（违反仓库红线 4 `{success}` 约定）；其语义状态码（soft_capped 等）取值入 P3 | — |
...
四份方案评审完成：元宝 ✅（v1.0）、豆包 ✅（v1.1）、智谱清言 ✅（v1.2/v1.3）、千问 ✅（v1.4，含任务清单文件处置）。**路线与排期冻结，P0 可即刻开工。**

## 6. 实施排期（v1.4 终版：落点到文件级，可开工）
...
| P1-5 | suppression_metrics 日表 + daily report 追加抑制段落（collected/deduped/cooling_died/delivered/digested + **dedup_compression_ratio**） | `db.ts` + `reports.ts` | 元宝 T6 简化版 + 智谱观测仪表（含「若配额 30 生效今日超额」模拟值）+ 千问压缩比指标 |
...
| # | 任务 | 说明 |
|---|---|---|
| P3-1 | `attention_budget` + daily 精选硬上限（初始配额 = 近 14 天日均消化 ×1.5，clamp 1~10）+ 首屏配额条；**千问形态**：软上限（80% 后仅 hook 级）/硬上限（100% 停止）渐进降级；feed 响应内联 `total_available_but_hidden`；**digest/周报豁免计额**；观测 `budget_used_pct`（60-85% 健康区间） | 前置：P2 累计 ≥50 条信号 |
...
- 2026-09-26 v1.4（终版）：合并千问方案评审（方案+任务清单两份 docx）。判定：产品与度量视野最完整但产品错位（按多用户 RSS/资讯 SaaS 设计 vs 单用户本地零推送纯拉取），模块 C 整层/emergency 预算/反馈全家/API 信封等否决；六处真增量收编（total_available_but_hidden 内联、soft/hard 双上限、digest 豁免计额、compression_ratio 与 budget_used_pct 指标、p70 备选、反馈权重规则入 backlog）。§0 升四方案结论（含四案共识三条）、§4 扩四方案映射（18 行）、§5 闭圈。路线与排期冻结。
- 2026-09-26 **v0.1.6**：定稿发布。版本号对齐（根/server/web 三 package.json + lockfile 均升至 0.1.6）；评审过程文档（四方案原文、千问评审存档、superpowers 实施计划、web-clip M1/M2 过程计划）清理，本文档更名为 `attention-budget-v0.1.6-plan.md` 作为唯一方案定稿。
