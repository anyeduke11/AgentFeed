<template>
  <div class="tagzone">
    ...
    <div class="sect">
      <div class="sect-head">
        <span class="sq"></span><h2 class="stitle">标签墙</h2><span class="sect-en">Tag Wall</span>
      ...
      <div class="legend">
        ...
        <span class="lg"><span class="dot" style="background:var(--steel)"></span>手动添加</span>
        <span class="lg"><span class="dot" style="background:var(--ok)"></span>蒸馏生成</span>
    ...
    <div class="sect">
      <div class="sect-head">
        <span class="sq"></span><h2 class="stitle">标签树</h2><span class="sect-en">Tag Tree</span>
      ...
      <div class="tagtree" v-if="tagTree.length || orphanSecondary.length">
        <div v-for="node in tagTree" :key="node.id" class="tt-node">
          <div class="tt-parent">
            <span class="tagc tagpick" :title="`领域「${node.name}」· 查看标签详情`" @click="openTag(node)"><span class="dot" :style="{ background: node.domain_color || 'var(--steel)' }"></span>{{ node.name }} <span class="cnt mono">{{ node.file_count ?? 0 }}</span></span>
          </div>
    ...
    <div class="sect" v-if="selected">
      <div class="sect-head">
        <span class="sq"></span><h2 class="stitle">标签详情</h2><span class="sect-en">Tag Detail</span>
    ...
    <div class="sect">
      <div class="sect-head">
        <span class="sq"></span><h2 class="stitle">标签治理</h2><span class="sect-en">Tag Governance</span>
...
// 标签治理面板：标签墙 / 标签树 / 标签详情 / 治理审计（Domains.vue 拆分，~460 行独立内聚域）
...
/** 标签来源色：与图例、状态点同源（钢蓝=手动 · 绿=蒸馏 · 墨黑=规则） */
function sourceColor(s?: string) {
  return s === 'manual' ? 'var(--steel)' : s === 'llm' ? 'var(--ok)' : 'var(--ink)'
}
